SimA=read.csv("/Users/philipayates/Dropbox/Phil Yates/sim-modelA.csv")
#Study first X column
library(dplyr)
library(ggplot2)
library(FAdist)
#ggplot(SimA, aes(x=t, y=X)) + geom_point() + ylab("Simulated Observations from GEV")
...
trunsLL=function(tseq,yinput){
  ...
  for(i in 1:length(tseq)) {
    conv[i]=optim(par=c(eta.A[i],log(tau.A[i]),kappa.A[i]),control=list(maxit=2000),local_like,t=tseq[i],h=0.30,y=yinput)$convergence
    outparm[i,1:3]=optim(par=c(eta.A[i],log(tau.A[i]),kappa.A[i]),control=list(maxit=2000),local_like,t=tseq[i],h=0.30,y=yinput)$par
    loc <- outparm[i,1]
...
trunsRLL=function(tseq,yinput){
  ...
  for(i in 1:length(tseq)) {
    ...
    istar=max(1,lowerl):min(n,upperl)
    parm_init=c(eta.A[i],log(tau.A[i]),kappa.A[i])
    ictr=1
...
for(i in 1:1000){
  sim <- i
  truns <- trunsLL(t,SimA[,i+1])
  out.truns <- cbind(rep(sim,100),truns)
  outAX <- rbind(outAX,out.truns)
}

colnames(outAX)[1] <- "sim"
write.table(outAX,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/outAX-h30.csv",sep=",",row.names=FALSE)

outAXR <- NULL
for(i in 1:1000){
  sim <- i
  truns.RLL <- trunsRLL(t,SimA[,i+1])
  out.truns.RLL <- cbind(rep(sim,100),truns.RLL$est)
  outAXR <- rbind(outAXR,out.truns.RLL)
}

colnames(outAXR)[1] <- "sim"
write.table(outAXR,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/outAXR-h30.csv",sep=",",row.names=FALSE)
