SimD=read.csv("/Users/philipayates/Dropbox/Phil Yates/sim-modelD.csv")
...
for(i in 1:1000){
  sim <- i
  truns <- trunsLL(t,SimD[,i+1])
  out.truns <- cbind(rep(sim,100),truns)
  outDX <- rbind(outDX,out.truns)
}

colnames(outDX)[1] <- "sim"
write.table(outDX,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/outDX-h30.csv",sep=",",row.names=FALSE)

# Redo 737? 955? for h=0.15
# Redo 184? for h=0.3
outDXR <- NULL
for(i in 185:1000){
  sim <- i
  truns.RLL <- trunsRLL(t,SimD[,i+1])
  out.truns.RLL <- cbind(rep(sim,100),truns.RLL$est)
  outDXR <- rbind(outDXR,out.truns.RLL)
}

colnames(outDXR)[1] <- "sim"
write.table(outDXR,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/outDXR-h30.csv",sep=",",row.names=FALSE)
