SimB=read.csv("/Users/philipayates/Dropbox/Phil Yates/sim-modelB.csv")
#Study first X column
library(dplyr)
library(ggplot2)
library(FAdist)
#ggplot(SimB, aes(x=t, y=X)) + geom_point() + ylab("Simulated Observations from GEV")
...
for(i in 1:1000){
  sim <- i
  truns <- trunsLL(t,SimB[,i+1])
  out.truns <- cbind(rep(sim,100),truns)
  outBX <- rbind(outBX,out.truns)
}

colnames(outBX)[1] <- "sim"
write.table(outBX,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/outBX-h30.csv",sep=",",row.names=FALSE)

# Retry for i=64 and 245 when h=0.15?
outBXR <- NULL
for(i in 1:1000){
  sim <- i
  truns.RLL <- trunsRLL(t,SimB[,i+1])
  out.truns.RLL <- cbind(rep(sim,100),truns.RLL$est)
  outBXR <- rbind(outBXR,out.truns.RLL)
}

colnames(outBXR)[1] <- "sim"
write.table(outBXR,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/outBXR-h30.csv",sep=",",row.names=FALSE)
