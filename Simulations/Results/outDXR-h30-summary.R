outDXR.h30 <- read.csv("~/Dropbox/Phil Yates/Sim Results/outDXR-h30.csv")

library(tidyverse)
outDXR.h30.summary <- outDXR.h30 %>%
group_by(tseq) %>%
...
# Calculate eta.D, tau.D, kappa.D
tseq <- outDXR.h30.summary$tseq
eta.D <- 3*max(0.25*tseq+0.1*dnorm(tseq,0.5,0.1)+0.1*dnorm(tseq,1,0.1))
tau.D <- max(exp(-1.5+0.2*tseq))
kappa.D <- -0.1

# Calculate the 0.99-quantile for Model D (the "contaminated" version of Model B)
...
for(i in 1:100){
  t <- outDXR.h30.summary$tseq[i]
  eta.B <- 0.25*t+0.1*dnorm(t,0.5,0.1)+0.1*dnorm(t,1,0.1)
  tau.B <- exp(-1.5+0.2*t)
  kappa.B <- -0.1
  #if(i<=99){
  true.Q99[i] <- qmixgev(0.99,0.9,eta=c(eta.B,eta.D),tau=c(tau.B,tau.D),
                         kappa=c(kappa.B,kappa.D))
  true.Median[i] <- qmixgev(0.5,0.9,eta=c(eta.B,eta.D),tau=c(tau.B,tau.D),
                            kappa=c(kappa.B,kappa.D))
  true.Mean[i] <- 0.9*(eta.B+tau.B*(gamma(1-kappa.B)-1)/kappa.B)+0.1*(eta.D+tau.D*(gamma(1-kappa.D)-1)/kappa.D)
  #}
  #if(i>99){
...
outDXR.h30.summary <- outDXR.h30.summary %>%
...
write_csv(outDXR.h30.summary,"/Users/philipayates/Dropbox/Phil Yates/Sim Results/Summaries/outDXR-h30.csv")
